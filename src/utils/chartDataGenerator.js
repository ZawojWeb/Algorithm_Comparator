...
import { hybridSortWrap, resetHybrid, hybridComparitionCount, hybridSwapsCount } from './hybridSort'
...
export const avgComparitions = (whichAlgo, howManyRepeats) => {
  ...
  if (whichAlgo[4]) {
    ...
    for (let index = 100; index <= 1000; index += 100) {
      ...
      avg = 0
      for (let j = 0; j < howManyRepeats; j++) {
        hybridSortWrap(generateStartArray)
        avg += hybridComparitionCount
      }
      avg = avg / howManyRepeats
      hybrydSortAvarages.push(avg)
    }
  }
